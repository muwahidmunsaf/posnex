<div class="d-flex" style="min-height: 100vh; overflow: hidden;">
    <div class="d-flex flex-column flex-shrink-0 p-3 bg-light border-end shadow-sm"
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            @if(auth()->check() && in_array(auth()->user()->role, ['admin', 'superadmin']))
                <li class="nav-item mt-2">
                    <a class="nav-link d-flex justify-content-between align-items-center {{ request()->is('users*') || request()->is('activity-logs*') || request()->is('recycle-bin*') || request()->is('manage-backup*') || request()->is('admin/reset-data*') ? '' : 'collapsed' }}"
                       data-bs-toggle="collapse" href="#settingsMenu"
                       aria-expanded="{{ request()->is('users*') || request()->is('activity-logs*') || request()->is('recycle-bin*') || request()->is('manage-backup*') || request()->is('admin/reset-data*') ? 'true' : 'false' }}">
                        <span><i class="bi bi-gear me-2"></i> Settings</span>
                        <i class="bi bi-chevron-down"></i>
                    </a>
                    <div class="collapse {{ request()->is('users*') || request()->is('activity-logs*') || request()->is('recycle-bin*') || request()->is('manage-backup*') || request()->is('admin/reset-data*') ? 'show' : '' }}" id="settingsMenu">
                        <ul class="nav flex-column ps-4">
                            <li>
                            ...
                            <li>
                                <a href="{{ route('activity-logs.index') }}" class="nav-link {{ request()->is('activity-logs*') ? 'active' : 'text-dark' }}">
