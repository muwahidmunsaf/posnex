        ...
        <div class="d-flex flex-wrap align-items-center gap-2" style="gap: 0.7rem;">
            ...
            <div id="searchPopover" class="card shadow p-3" style="position:absolute;right:60px;top:60px;min-width:220px;z-index:1050;display:none;">
                <form method="GET" action="{{ route('distributors.index') }}" class="mb-0" id="distributor-name-search-form-popover">
                    ...
                    <a href="{{ route('distributors.index') }}" class="btn btn-sm btn-secondary w-100 mt-2">Clear</a>
                </form>
            </div>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    ...
                    <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteDistributorModal{{ $distributor->id }}" title="Delete">
                        <i class="bi bi-trash"></i>
                    </button>
                    <!-- Hidden form for deletion -->
                    <form action="{{ route('distributors.destroy', $distributor) }}" method="POST" style="display:none;" id="deleteDistributorForm{{ $distributor->id }}">
                        @csrf @method('DELETE')
                    </form>
                    <!-- Modal (outside form) -->
                    <div class="modal fade" id="deleteDistributorModal{{ $distributor->id }}" tabindex="-1" aria-labelledby="deleteDistributorModalLabel{{ $distributor->id }}" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header bg-danger text-white">
                                    <h5 class="modal-title" id="deleteDistributorModalLabel{{ $distributor->id }}">Confirm Deletion</h5>
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-danger" onclick="document.getElementById('deleteDistributorForm{{ $distributor->id }}').submit();">Yes, Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
if (printAllBtn) {
    printAllBtn.addEventListener('click', function(e) {
        ...
        let url = '{{ route('distributors.printAll') }}';
