    ...
    <div class="row g-4 align-items-stretch">
        <div class="col-12 col-lg-6">
            <div class="card shadow-sm h-100 border-0 bg-light">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.csv-backup.export') }}" method="POST" id="backup-form">
                        @csrf
        ...
        <div class="col-12 col-lg-6 d-flex flex-column gap-4">
            <div class="card shadow-sm border-0 bg-danger bg-opacity-10 h-100 mb-0">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.full-backup') }}" method="POST">
                        @csrf
            ...
            <div class="card shadow-sm border-0 bg-info bg-opacity-10 h-100 mb-0">
                ...
                <div class="card-body">
                    @if(isset($cloudBackup) && $cloudBackup)
                        <div class="d-flex align-items-center mb-2">
                            ...
                            <div>
                                <strong>Connected Google Account:</strong> {{ $cloudBackup->name }} ({{ $cloudBackup->email }})
                            </div>
                        </div>
                        <a href="{{ route('cloud-backup.google.redirect') }}" class="btn btn-outline-primary btn-sm mb-3"><i class="bi bi-google"></i> Change Google Drive Account</a>
                        <hr>
                        ...
                        @if($cloudBackup->last_run_at)
                            <div class="mt-3 text-muted"><i class="bi bi-clock-history"></i> Last backup run: <span class="badge bg-light text-dark">{{ $cloudBackup->last_run_at->format('Y-m-d H:i') }}</span></div>
                        @endif
                    @else
                        <p>No Google Drive account connected.</p>
                        <a href="{{ route('cloud-backup.google.redirect') }}" class="btn btn-success"><i class="bi bi-google"></i> Connect Google Drive Account</a>
                    @endif
                </div>
            </div>
        </div>
